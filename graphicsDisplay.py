...
class PacmanGraphics:
    def __init__(self, zoom=1.0, frameTime=0.0, capture=False, hunters=False):
        self.have_window = 0
    ...
    def drawStaticObjects(self, state):
        ...
        if self.hunters:
            # self.drawWalls(layout.redWalls, GHOST_COLORS[0])
            # self.drawWalls(layout.blueWalls, GHOST_COLORS[1])
            self.food = []
        else:
    ...
    def update(self, newState):
        ...
        if self.hunters:
            self.drawWalls(self.layout.redWalls, GHOST_COLORS[0], newState.observedPositions)
            self.drawWalls(self.layout.blueWalls, GHOST_COLORS[1], newState.observedPositions)
    ...
    def isWall(self, x, y, walls, obsMatrix=None):
        if x < 0 or y < 0:
            return False
        ...
        if not self.hunters:
            return walls[x][y]
        seenColoredWall = obsMatrix and obsMatrix[x][y] and (self.layout.redWalls[x][y] or self.layout.blueWalls[x][y])
        return walls[x][y] or self.layout.redWalls[x][y] or self.layout.blueWalls[x][y]
...
class FirstPersonPacmanGraphics(PacmanGraphics):
    def __init__(self, zoom = 1.0, showGhosts = True, capture = False, frameTime=0, hunters=False):
        PacmanGraphics.__init__(self, zoom, frameTime=frameTime, hunters=hunters)
